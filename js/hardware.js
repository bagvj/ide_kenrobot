...
define(["jquery","jsplumb","eventcenter","d3","flowchart_item_set","jquery-ui"],function($,jsPlumb,eventcenter,d3,fis){
		...
		jsPlumb.ready(function(){
			...
			$('div.'+itemClass).attr('draggable','true').on('dragstart', function(ev){
				...
				for(var i=0;i<linkableEndpoints.length;i++){
					linkableEndpoints[i].setPaintStyle({fillStyle:'#333'});
				}
				linkableEndpoints=[];
				linkablePortBits={};
			}).on('drag',function(ev){
	...
	function onDragoverEvent(e,obj){
		...
		var closeDistance=0;
		for(var i=0;i<linkableEndpoints.length;i++){
			linkableEndpoints[i].setPaintStyle({fillStyle:'#FF0'});
		...
		if(closeEndpoint!=null){
			closeEndpoint.setPaintStyle({fillStyle:'#F00'});
		...
		if(tmpJsPlumbNode.index > -1){
			...
			if (tmpNode['add_info']['usedPortBit'] && tmpNode['add_info']['usedPortBit'].length>0) {
				...
				if (mainboard != null) {
					...
					var usedPortIndex=0;
					switch (usedPort) {
						case 'A':usedPortIndex=0;break;
						case 'B':usedPortIndex=1;break;
						case 'C':usedPortIndex=2;break;
						case 'D':usedPortIndex=3;break;
						case 'E':usedPortIndex=4;break;
						case 'F':usedPortIndex=5;break;
						case 'G':usedPortIndex=6;break;
						case 'H':usedPortIndex=7;break;
						default:usedPortIndex=0;break;
					}
	...
	function finishDrag(e){
		...
		if (targetDiv.closest("div").attr('class').indexOf('_jsPlumb') >= 0 ) {
			...
			if(sourceFis.kind=="hardware" && (sourceFis.type=='board')){
				...
				var conns=jsPlumb_instance.getConnections({source:targetDiv.closest("div")});

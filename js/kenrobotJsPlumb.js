...
define(["jquery","jsplumb","eventcenter","d3","flowchart_item_set","generateC","jquery-ui","jquery-menu"],function($,jsPlumb,eventcenter,d3,fis,generateC){
	...
	function deleteNode(node){
		...
		$.each(jsPlumb_instance.getConnections({source:$(node)}),function(i,o){
			if(o.targetId!=o.sourceId){
				childrenEndpoints.push(o.endpoints[1]);
			}
		});
		$.each(jsPlumb_instance.getConnections({target:$(node)}),function(i,o){
			if(o.targetId!=o.sourceId){
				parentEndpoints.push({endpoint:o.endpoints[0],label:o.getLabel()});
			}
		});
		...
		$.each(parentEndpoints,function(i,o){
			...
			tmpConnector.setLabel({location:0.1,label:o.label});
		});
	}
	...
	function initJsPlumbInstance(){
		...
		jsPlumb_instance = jsPlumb.getInstance({     
			Connector : [ "Flowchart", { gap:3,curviness:50,midpoint:1,stub:20} ],
			...
			connectorStyle:{outlineWidth:2,outlineColor:"WHITE"}
	    });
	...
	function initConnection(node,target,centerX,centerY){
		...
		switch(sourceEndPoint.anchor.type){
			case "TopCenter":objX=baseX;objY=baseY-$(targetDiv).outerHeight()-20;break;
			case "RightMiddle":objX=baseX+$(node).outerWidth()+20;objY=baseY+$(targetDiv).outerHeight()+20;break;
			case "BottomCenter":objX=baseX;objY=baseY+$(targetDiv).outerHeight()+30;break;
			case "LeftMiddle":objX=baseX-$(node).outerWidth()-20;objY=baseY+$(targetDiv).outerHeight()+20;break;
			default:break;
		}
		...
		for(var i=0;i<connections.length;i++){
			...
			for(var j=0;j<arrSourceTargetEndPoint.length;j++){
				...
				if(tmpObjType && tmpObjType=="if"){
					if(j==0){
						conn.setLabel({location:0.1,label:"N"});
					}else if(j==1){
						conn.setLabel({location:0.1,label:"Y"});
					}
				}
			}
		}

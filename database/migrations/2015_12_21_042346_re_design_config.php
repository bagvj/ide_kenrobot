...
class ReDesignConfig extends Migration {
	...
	public function up() {
		...
		DB::statement("insert  into `components`(`id`,`name`,`label`,`in_use`,`auto_connect`,`width`,`height`,`varName`,`varCode`,`headCode`,`setupCode`,`category`) values (1,'buzzer','蜂鸣器',1,0,85,80,'buzzer_','int $NAME = $P0;\n','','','one-port-bottom'),(2,'servo','舵机',1,0,125,106,'servo_','Servo $NAME;\n','#include <Servo.h>\n','$NAME.attach($P0);\n','one-port-bottom'),(3,'lcd','1602',1,1,170,92,'lcd_','LiquidCrystal $NAME(0);\n','#include <Wire.h>\n#include <BitbloqLiquidCrystal.h>\n','$NAME.begin(16, 2);\n$NAME.clear();\n','two-port-top'),(4,'led','LED',1,0,55,83,'led_','int $NAME = $P0;\n','','pinMode($NAME, OUTPUT);\n','one-port-bottom'),(5,'ultrasoundSensor','超声波',1,0,120,79,'ultrasound_sensor_','US $NAME($P0, $P1);\n','#include <US.h>\n#include <Servo.h>\n#include <SoftwareSerial.h>\n#include <Wire.h>\n','','two-port-bottom'),(6,'button','按钮',1,0,90,73,'button_','int $NAME = $P0;\n','','pinMode($NAME, INPUT);\n','one-port-bottom'),(7,'buttonPanel','按键矩阵',1,0,165,120,'button_panel_','ButtonPad $NAME($P0);\n','#include <ButtonPad.h>\n#include <Servo.h>\n#include <SoftwareSerial.h>\n#include <Wire.h>\n','','one-port-top'),(8,'infraredSensor','光电对管',1,0,90,78,'infrared_sensor_','int $NAME = $P0;\n','','pinMode($NAME, INPUT);\n','one-port-bottom'),(9,'lineFollower','循迹矩阵',1,0,95,87,'line_follower_','LineFollower $NAME($P0, $P1);\n','#include <LineFollower.h>\n#include <Servo.h>\n#include <SoftwareSerial.h>\n#include <Wire.h>\n','','two-port-bottom'),(10,'joystick','摇杆',1,0,100,101,'joystick_','Joystick $NAME($P1, $P2, $P0);\n','#include <Joystick.h>\n#include <Wire.h>\n#include <Servo.h>\n#include <SoftwareSerial.h>\n','','joystick'),(11,'lightSensor','光敏电阻',1,0,90,64,'light_sensor_','int $NAME = $P0;\n','','pinMode($NAME, INPUT);\n','one-port-top'),(12,'potentiometer','电位器',1,0,65,103,'potentiometer_','int $NAME = $P0;\n','','pinMode($NAME, INPUT);\n','one-port-top'),(13,'bluetooth','蓝牙模块',1,0,115,88,'bluetooth_','bqSoftwareSerial $NAME($P0, $P1, 9600);\n','#include <SoftwareSerial.h>\n#include <bqSoftwareSerial.h>\n#include <Servo.h>\n#include <Wire.h>\n','','two-port-bottom'),(14,'serialPort','串口模块',1,1,115,71,'serial_port_','bqSoftwareSerial $NAME(0, 1, 9600);\n','#include <SoftwareSerial.h>\n#include <bqSoftwareSerial.h>\n#include <Servo.h>\n#include <Wire.h>\n','','one-port-right');");
